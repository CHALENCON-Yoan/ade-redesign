  ...
  if (localStorage.getItem("weekDisplay") == "true") {
    ...
    if (LOCALHOST) {
      ...
      const request = await fetch(
        `${BASE_URL}?function=getEvents&projectId=${PORJECT_ID}&data=${PROJECT_DATA}&weeks=${weekId}&resources=${resource}&detail=8&days=0%7C1%7C2%7C3%7C4`
      );
    ...
    if (LOCALHOST) {
      ...
      const request = await fetch(
        `${BASE_URL}?function=getEvents&resources=${resource}&projectId=${PORJECT_ID}&data=${PROJECT_DATA}&detail=8&days=0&date=${convertDate(
          date
        )}`
      );
