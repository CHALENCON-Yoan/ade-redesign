async function displayLessons(date, resource) {
    if (IS_IT_CLASSROOMS_PAGE) {
        await displayITClassroomsLessons(date, resource);
        return;
    }
...
function setClassroomNames() {
    const days = document.querySelectorAll(".day");
    itClassrooms.forEach((itClassroom, index) => {
        const day = days[index];
        day.textContent = itClassroom.name;
    });
}

async function displayITClassroomsLessons(date, resource) {
    let multiResourcesDayTree;

    let frenchDate = convertFrenchDate(date);

    if (LOCALHOST) {
        multiResourcesDayTree = itClassroomsExample;
    } else {
        const request = await fetch(
            `${BASE_URL}?function=getEvents&resources=${resource}&projectId=${PROJECT_ID}&data=${PROJECT_DATA}&detail=8&days=0&date=${convertDate(
                date,
        ...
        multiResourcesDayTree = await request.text();
    }

    const parser = new DOMParser();
    const xmlDoc = parser.parseFromString(multiResourcesDayTree, "text/xml");

    displayITClassroomsDay(xmlDoc, frenchDate);
}

function displayITClassroomsDay(xml, date) {
    let events = Array.from(xml.getElementsByTagName("event"));
    events = events.filter((event) => {
        return event.getAttribute("date") === date;
    });
    ...
    itClassrooms.forEach((itClassroom, index) => {
        const classroomEvents = events.filter((event) => {
            const resources = Array.from(
                event.getElementsByTagName("resource"),
            );
            return resources.some(
                (resource) =>
                    resource.getAttribute("category") === "classroom" &&
                    parseInt(resource.getAttribute("id")) === itClassroom.id,
            );
        });

        if (classroomEvents.length === 0) {
            displayBlankDay(index + 1);
        } else {
            classroomEvents.forEach((event) => {
                displayEvent(event, index + 1);
            });
        }
    });
}
