async function getCalendar() {
    ...
    for (let i = 0; i < weeks.length; i++) {
        weekMap[weeks[i].getAttribute("date")] =
            weeks[i].getAttribute("id") - 1;
    }
}
...
async function makeDatalist() {
    ...
    leafs = leafs.filter((leaf) => {
        return (
            ...
            (leaf
                .getAttribute("path")
                .startsWith("CAMPUS Valence.VALENCE-IUT") ||
                leaf
                    .getAttribute("path")
                    .startsWith("CAMPUS VALENCE.VALENCE-IUT") ||
    ...
    leafs.forEach((leaf) => {
        resourcesMap[leaf.getAttribute("name")] = parseInt(
            leaf.getAttribute("id"),
        );
...
async function setNextLessonsDate() {
    ...
    await displayLessons(
        currentDate,
        IS_IT_CLASSROOMS_PAGE
            ? IT_CLASSROOMS_RESOURCE_ID
            : localStorage.getItem("lastResource"),
    );
}

async function getITClassrooms() {
    if (!IS_IT_CLASSROOMS_PAGE) {
        return;
    }
    ...
    leafs = leafs.filter((leaf) => {
        return (
            leaf.getAttribute("category") === "classroom" &&
            (leaf
                .getAttribute("path")
                .startsWith("CAMPUS VALENCE.VALENCE-IUT.Spécialisées.Linux") ||
                leaf
                    .getAttribute("path")
                    .startsWith("CAMPUS VALENCE.VALENCE-IUT.Informatique")) &&
            !leaf.getAttribute("path").includes("temporaire")
        );
    });
    ...
    leafs.forEach((leaf) => {
        const itClassroom = {
            id: parseInt(leaf.getAttribute("id")),
            name: leaf.getAttribute("name"),
        };
        itClassrooms.push(itClassroom);
    });
}
